...
// StatusInfo represents response of a2.tellStatus
type StatusInfo struct {
	...
	NumSeeders      string         `json:"numSeeders"`      // The number of seeders a2 has connected to. BitTorrent only.
	...
	Connections     string         `json:"connections"`     // The number of peers/servers a2 has connected to.
	...
	Files           []FileInfo     `json:"files"`           // Returns the list of files. The elements of this list are the same structs used in a2.getFiles() method.
...
// URIInfo represents an element of response of a2.getUris
type URIInfo struct {
...
// FileInfo represents an element of response of a2.getFiles
type FileInfo struct {
	...
	CompletedLength string    `json:"completedLength"` // Completed length of this file in bytes. Please note that it is possible that sum of completedLength is less than the completedLength returned by the a2.tellStatus() method. This is because completedLength in a2.getFiles() only includes completed pieces. On the other hand, completedLength in a2.tellStatus() also includes partially completed pieces.
	...
	URIs            []URIInfo `json:"uris"`            // Returns a list of URIs for this file. The element type is the same struct used in the a2.getUris() method.
}

// PeerInfo represents an element of response of a2.getPeers
type PeerInfo struct {
	...
	AmChoking     string `json:"amChoking"`     // true if a2 is choking the peer. Otherwise false.
	PeerChoking   string `json:"peerChoking"`   // true if the peer is choking a2. Otherwise false.
...
// ServerInfo represents an element of response of a2.getServers
...
// GlobalStatInfo represents response of a2.getGlobalStat
...
// VersionInfo represents response of a2.getVersion
type VersionInfo struct {
	Version  string   `json:"version"`         // Version number of a2 as a string.
	Features []string `json:"enabledFeatures"` // List of enabled features. Each feature is given as a string.
}

// SessionInfo represents response of a2.getSessionInfo
type SessionInfo struct {
	Id string `json:"sessionId"` // Session ID, which is generated each time when a2 is invoked.
}
